...
function main() {
    ...
    const sitio_1: StreamPlatform = new StreamPlatform (
        'RTVC Play (TV pública Colombia)',
        'https://www.rtvcplay.co/'
    )

    const sitio_2: StreamPlatform = new StreamPlatform (
        'Netflix',
        'https://www.netflix.com/co/'
    )
    
    const sitio_3: StreamPlatform = new StreamPlatform( )

    const plan_A: Plan = new Plan (
        0,
        'Usuario Basico',
        'El acceso es abierto a todos lo usuarios dentro del terriotorio de Colombia',
        sitio_1
    )

    const plan_B: Plan = new Plan(
        16900,
        'Basico',
        `El cliente contará con 'Buena' calidad de video en resolución de 720p y multi-dispositivo`,
        sitio_2
    )

    const plan_C: Plan = new Plan ()

    const director_A: Director = new Director (
        `Rob Corn`,
        'https://static.wikia.nocookie.net/greysanatomy/images/2/24/13x03BTS2.jpg/revision/latest/scale-to-width-down/180?cb=20160920003107',
        `He is the most frequently director in the episodes of Grey's Anatomy `
    )

    const serie_1: SeriesTV = new SeriesTV(
        `Grey's Anatomy`,
        'https://static.tvmaze.com/uploads/images/medium_portrait/424/1061065.jpg'           
    )
    
    const episode_1: Episode = new Episode(
        `A hard day's night`,
    ...
    const categoria_A : Category = new Category (
        'Drama'
    )

    const categoria_B : Category = new Category ( )

    const performer_A: Performer = new Performer (
        `Ellen Pompeo`,
        `https://static.tvmaze.com/uploads/images/medium_portrait/2/5773.jpg`,
        `She is main character on Grey's Anatomy`
    )   
    ...
    const seriados: Array<SeriesTV> = [serie_1]
    const plataformas: Array<StreamPlatform> = [sitio_1, sitio_2, sitio_3]
    const categorias: Array<Category> = [categoria_A, categoria_B]
